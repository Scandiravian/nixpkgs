...
stdenv.mkDerivation rec {
  ...
  buildPhase = ''
    ...
    # Upstream issue https://github.com/gristlabs/grist-core/issues/1399
    yarn --offline --frozen-lockfile --ignore-platform --prod --ignore-engines --no-progress --non-interactive add chokidar
    patchShebangs node_modules
  ...
  installPhase = ''
    ...
    cp -r _build "$out/libexec"
    cp -r node_modules "$out/libexec"
    cp -r plugins "$out/libexec"
    cp -r sandbox "$out/libexec"
    cp -r static "$out/libexec"
    cp -r bower_components "$out/libexec"
